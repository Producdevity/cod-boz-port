...
usage() {
  echo "Usage: $0 [--roms|--roms2|--root PATH] <ssh-host>" >&2
  echo "Defaults to /roms, which is the normal DarkOS/ArkOS PortMaster storage path." >&2
  echo "Or set CODBOZ_DARKOS_DEPLOY_HOST and optionally CODBOZ_DARKOS_DEPLOY_ROOT." >&2
}
...
host="${host:-${CODBOZ_DARKOS_DEPLOY_HOST:-}}"
if [ -z "$host" ]; then
  usage
  exit 2
fi
